...
AC_SUBST([bindir],         ['${exec_prefix}'])dnl
...
do
  ...
  AC_MSG_ERROR([expected an absolute directory/file name for $ac_var: $ac_val])
done
...
AS_HELP_STRING([prefix=PREFIX        ], [Install Quafios files to PREFIX.])
AS_HELP_STRING([                     ], [Default Value: $ac_default_prefix])
