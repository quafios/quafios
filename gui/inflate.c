...
static unsigned char *input_stream;
uint8_t  in[2048];   /* in buffer.   */
uint32_t inp = 0;    /* in pointer.  */
uint8_t  inb = 1;    /* in bit.      */
uint32_t sector = 0; /* sector index */
uint32_t rem = 0;    /* remaining    */

uint8_t  *out;
int32_t  outp = 0;   /* out pointer. */

void streamInit(unsigned char *input, int size, unsigned char *buf) {
    ...
    /* print progress: */
    /*printf("Decompressing...", rem);*/

    /* load first sector: */
    sectsize = rem >= 2048 ? 2048 : rem;
    for (i = 0; i < sectsize; i++)
        in[i] = input_stream[sector*2048 + i];
...
uint8_t getNextBit() {
    ...
    if (inp == 2048) {
        /* read next sector */
        int i;
        int sectsize = rem >= 2048 ? 2048 : rem;
        inp = 0;
        sector++;
        for (i = 0; i < sectsize; i++)
            in[i] = input_stream[sector*2048 + i];
...
char *trees[TREE_BYTES*5];

#define TREE_BASE(N)    ((node *)(trees + TREE_BYTES*N))
...
int inflate(unsigned char *input, int size, unsigned char *buf) {

    /* initialize streams: */
    streamInit(input, size, buf);
